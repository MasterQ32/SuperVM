...
void dump_vm()
{
	...
	for (uint32_t i = 1; i <= MIN(VM_STACKSIZE, mainCore.stackPointer); i++)
	{
...
void setup_device(int i, int intr, device_t *dev)
{
	if(dev == NULL) {
		fprintf(stderr, "Device %d not initialized!\n", i);
		return;
	}
	devices[i] = dev;
	deviceConfigs[i].interruptBase = intr;
	devices[i]->tagPtr = &deviceConfigs[i];
}
